...
def game_loop(args, filename, scenario):
    ...
    try:
        ...
        # Setting up the client, traffic manager and world
        ...
        ### VELNote
        ### Spawn vehicles and walkers based on user arguments
        ### Note: It is recommended to run generate_traffic.py instead of using the spawn vehicle argument.
        ### Note: Vehicles spawned here have poor autopilot capabilities.
        if args.spawnvehicles < 0:
            print("Error: Number of vehicles cannot be negative.")
            return
        if args.spawnwalkers < 0:
            print("Error: Number of walkers cannot be negative.")
            return      
        if args.spawnvehicles > 0:
            spawn_random_vehicles(sim_world, client, traffic_manager, args.spawnvehicles)
        if args.spawnwalkers > 0:
            spawn_random_walkers(sim_world, client, args.spawnwalkers)

        #Get list of vehicle and walkers location to note
        ...
        ### VELNote
        ### Declare DataExporter, SafetyMetrics for analysis
        ### Declare Attacker for automatically attack
        data_exporter = DataExporter()
        safety_metrics = SafetyMetrics()
        attacker = Attacker()


        ### Define the automatic agent in three different behaviours
        ### Basic: 
        ### Constant:    
        ### Behavior: 
        if args.agent == "Basic":
...
def main():
    ...
    argparser.add_argument('--host',metavar='H',default='192.168.56.1',help='IP of the host server (default: 127.0.0.1)')
    ...
    argparser.add_argument('--spawnvehicles',metavar='N',default=0,type=int,help='Number of vehicles (default: 10)')
    argparser.add_argument('--spawnwalkers',metavar='W',default=0,type=int,help='Number of walkers (default: 10)')
